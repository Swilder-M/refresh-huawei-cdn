...
def hmacsha256(keyByte, message):
    return hmac.new(keyByte.encode('utf-8'), message.encode('utf-8'),
                    digestmod=hashlib.sha256).digest()


# Create a "String to Sign".
def StringToSign(canonicalRequest, t):
    _bytes = HexEncodeSHA256Hash(canonicalRequest.encode('utf-8'))
    return f'{Algorithm}\n{datetime.strftime(t, BasicDateFormat)}\n{_bytes}'
...
def AuthHeaderValue(signature, AppKey, signedHeaders):
    return f'{Algorithm} Access={AppKey}, SignedHeaders={";".join(signedHeaders)}, Signature={signature}'
